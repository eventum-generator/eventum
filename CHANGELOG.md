...
- **The `clickhouse` output no longer slows every other generator down** — loading the plugin on free-threaded Python re-enabled the GIL process-wide, so all generators lost parallelism, not just the one writing to ClickHouse. The ClickHouse client now requires a version whose compiled modules keep the GIL disabled
- **TLS certificate paths work in the `clickhouse` output** — `ca_cert`, `client_cert` and `client_cert_key` failed with a type error while the configuration was read, which made certificate-based TLS unusable
