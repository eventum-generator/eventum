...
- **Studio redesign** — every screen is restyled on one design system: matched dark and light themes, consistent surfaces, controls, tables, menus and alerts, and a single status palette, so a running, finished, failed or idle instance reads the same on every page
- **The project page is now a development studio** — a generator opens as a docked workspace: a file explorer, a code editor with tabs, an inspector for the selected pipeline stage, and a console at the bottom holding the timestamp preview, event debugger, template state and formatter. The pipeline strip (Input → Event → Output) selects the stage, open files persist as tabs, one Save covers the configuration and every edited file, and the debug tools keep their results and form input while you move between stages
- **A broken generator config no longer locks the project** — when `generator.yml` fails to parse, the project opens in recovery mode with the error shown over the file editor; fixing and saving it revalidates the configuration and restores the normal view. The root config file can no longer be renamed, deleted or moved out of the project root
- **The instance page is now a live overview** — the opened instance is split into Overview, Settings and Logs tabs. Overview draws a live throughput graph over the pipeline (Input → Event → Output with per-plugin counters) next to the instance's project, mode, autostart, timezone and last run, and lists the scenarios it belongs to with inline add and remove. The header carries the live status and a ticking uptime alongside Start, Stop and Restart, Save appears only when the configuration has unsaved changes, and the log viewer is embedded in the page and streams only while its tab is open
- **Monitoring is a live dashboard** — an animated Input → Event → Output flow with per-stage metrics, throughput and failure charts over a rolling window, each instance's share of the output load, and CPU, memory, disk and network tiles
- **Runtime stats in the instances table** — running instances show Flow (average output EPS), Errors and Written as sortable columns, refreshed on demand
- **Filters, counts and empty states across the tables** — Projects filters by All / In use / Unused, Instances and Scenarios by All / Running / Inactive, each list shows how many records it holds and how many are active, and every table has a first-run and a no-match state. The filters are part of the URL, so a filtered view can be linked and survives a reload
- **Instance chips on the projects table** — each chip links to its instance and carries an activity dot; when a running instance is hidden behind the overflow chip, the overflow lights up too
- **Settings is split into sections** — Server, Generation, Paths and Logging now sit in a left rail and open one at a time instead of one long scroll; a section with unsaved edits is marked with a dot, Save is pinned in the page header, and each section has its own URL
- **Secrets gains an inline add form** — a new secret is entered in the panel header (name and value) instead of a row inside the table, the value is typed into a password field, and a secret name can be copied with one click
- **Management is an instance console** — the page now shows application and host identity (version, Python, platform, address, uptime), a CPU and memory snapshot, the application log streamed live in the page instead of behind a modal button, and a danger zone for restarting or stopping the instance
- **The scenario page gains a status header and inline template previews** — the header carries an aggregate status, instance counts, and Start all and Stop all; instance cards show a clearer read and write view, and a template preview opens inline with syntax highlighting instead of in a modal
- **Unsaved changes are guarded on every exit** — leaving an instance or a project with unsaved changes now asks for confirmation on any navigation (the sidebar, a link, the browser back button) and warns before a page refresh or a closed tab; previously only the back button was covered
...
- **Network and disk figures measure Eventum, not the whole host** — the Disk I/O and Network tiles reported system-wide counters, so they showed the activity of every process on the machine. Disk numbers now come from the Eventum process and network bytes are counted inside the application; CPU and memory stay host-level, and all counters are cumulative since startup
- **The write timeout in Settings accepts only whole seconds** — the field allowed fractional values that the configuration then rejected on save
