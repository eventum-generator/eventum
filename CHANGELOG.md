...
- **Reordered the instance overview from state to detail** — the live figures of the instance come first, then its throughput, then the stage-by-stage graph, then what it occupies, grouped by what each figure answers: the processor it takes and whether it waits for one, the memory its queues hold, and the bytes it moves. The resources were one flat grid of eight figures before
