...
- **Rebuilt the instance overview around what the instance is doing** — the page opens on one panel across its full width: the rate events enter and leave the pipeline at, what it has moved in total, and what that costs — the processor it takes and whether it waits for one, the memory its queues hold, the bytes it moves. Its throughput over the window sits beside what the instance is — project, mode, autostart, timezone, last run and the scenarios it belongs to, now one panel instead of two — and the stage-by-stage graph spans the full width below. The resources used to be a flat grid of eight figures at the foot of the page, under a column that ended half way up it
