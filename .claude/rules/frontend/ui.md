...
- **Semantics.** Danger, success, warning and info are the `red` / `green` / `yellow` / `blue` palettes. Terminal success has no palette of its own: a finished instance takes the neutral chip of a state at rest and keeps its green in the indicator alone, at a shade deep enough to read as switched off. Use the colour prop (`c="red"`, `color="green"`) over a raw variable wherever a Mantine component accepts one.
